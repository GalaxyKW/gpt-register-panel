...
登录框只接受与后端规则一致的 16–4096 位可打印 ASCII 管理员令牌，不接受中文、空格、换行或不可见字符，也不会自动裁剪或编码输入。“我已按强身份完成人工核对”只用于待对账确认，不是登录令牌。旧标签页缓存了格式错误的令牌时，新版页面会仅清除失效的认证缓存并请求重新登录；不要清空全部浏览器存储，以免丢失尚未确认的写操作幂等回执。重新登录不会解除待对账阻挡。

“本地导入预览”只针对活动目录中的本地 token，不是 Sub2API 账号测试的前置步骤。仅远程账号不能生成本地导入计划；各按钮下方会直接说明不可用原因。

“测试 Sub2API 账号”支持确认后自动分批：当前页面最多选择 1000 个目标，每批最多 100 个，上一批逐项完整成功后才提交下一批。确认框列出测试模型、账号 ID、批次数和不可测试项目；缺少强身份或当前测试凭证的账号不会自动补齐身份，只有明确勾选同意跳过后才测试其余账号。每批前重新核验任务清单与尚未执行目标的原始 revision，目标变化、失败、部分完成、未知结果或待对账都会停止后续批次。点击“停止后续批次”不会取消已经提交的后台任务；刷新或关闭页面不会自动续交尚未提交的批次，请先核对已提交任务结果，再重新选择剩余账号。批次之间不会自动更换目标凭证或重试失败请求。

### 本地 token 缺失时重新获取

使用“从本地账号获取 token”打开独立的 `username.json` 清单。此入口不使用上方远程表格的选择，也不会根据 Sub2API 邮箱自动关联本地登录资料。只有邮箱和手机号不重复、资料完整、非删除/停用/禁用终态且没有活动目录 token 的本地记录可以选择；历史备份不会用作身份授权，也不会阻止恢复。已有活动 token（即使过期）仍从账号表的“本地 Phase 3”运行。

本地清单按每页 100 条展示，每批最多提交 100 个，任务串行运行。提交前以及排队后的执行前均核验本地记录，资料变更会拒绝执行；缺少强身份的新输出、非预期文件变更或无法安全确认的密码重置会保留人工对账阻挡。新 token 获取成功后不会自动导入 Sub2API，仍需单独预览和确认导入。清单接口为 `GET /api/phase3/local-accounts`，独立提交接口为 `POST /api/phase3/local`；沿用认证、写入开关、幂等回执、账号 claim 和审计保护。
