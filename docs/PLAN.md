...
- 不修改 Sub2API 源码、数据库或正在运行的账号。
- 不修改 gpt_register 的 tokens、use_token、username.json 或浏览器 profile。
- 不启动 Phase 3，不触发邮箱验证码、浏览器登录或 token 刷新。
- 不创建或推送远程 Git 仓库；远程地址和认证方式确认后再配置。
...
完成差异预览、选择性导入、编号分配、幂等更新、备份、缓存刷新、健康检查和回滚演练。

### M4：Phase 3 任务

完成 gpt_register 非交互参数、worker、串行锁、超时、日志脱敏、token 身份校验和更新闭环。
...
7. 保留每次导入前的 SQL 压缩备份；在未另行指定前，计划按 30 天保留。
...
## 17. 下一步
先按 M1 做只读适配器和脱敏测试数据，不接触生产写入；验收后再开启 M3 导入和 M4 Phase 3。
